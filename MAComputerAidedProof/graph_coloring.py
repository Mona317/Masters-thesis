...
# Generates all colorings of two C5s, not regarding the edges running between the C5s, where the colorings of the first C5 are rotations of the coloring {'u_0': 1, 'u_1': 2, 'u_2': 1, 'u_3': 2, 'u_4': 3}, so color 3 appears only once. Every coloring of a C5 can be received from these by permuting the colors. The returned colorings are not necessarily proper.
...
# coloring_wo_c5 has to be a coloring of a graph that consists of k-1 C5s. The method generates all colorings for a k'th C5 and returns all combinations of them with the input coloring.
